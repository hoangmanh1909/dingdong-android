...
public class TimDuongDiFragmentBaoPhat extends ViewFragment<TimDuongDiContractBaoPhat.Presenter> implements TimDuongDiContractBaoPhat.View, OnMapReadyCallback, PermissionsListener {
...
//    CustomTextView tv_address_from;
    ...
    TimDuongDiAdapterBaoPhat mAdapter;
...
//    MarkerView markerView;
    ...
    private MainActivityLocationCallback mCallback = new MainActivityLocationCallback(this);

    public static TimDuongDiFragmentBaoPhat getInstance() {
        return new TimDuongDiFragmentBaoPhat();
    }

    @Override
    protected int getLayoutId() {
        Mapbox.getInstance(getViewContext(), getString(R.string.mapbox_access_token));
        return R.layout.fragment_tim_duong_di_bao_phat;
    }
    ...
    public void initLayout() {
        ...
        mAdapter = new TimDuongDiAdapterBaoPhat(getViewContext(), mList) {
            @Override
                            ...
                            public void run() {
                                ...
                                if (position != 0) {
                                    if (position < mList.size()) {
                                        ...
                                        mapBoxsdk.getMapAsync(TimDuongDiFragmentBaoPhat.this);
                                        hideProgress();
        ...
        ItemTouchHelper.Callback callback = new SimpleItemTouchHelperCallbackBaoPhat(mAdapter) {
            @Override
    ...
    public void showListSuccess(Object object) {
        ...
        try {
            ...
            if (paths.length() > 0) {
                ...
                tvKm.setText("Số km: " + mPresenter.getKM());
...
//                    }
                    ...
                    JSONArray element1 = coordinates.getJSONArray(coordinates.length() - 1);
                    Log.d("THanhkhiemOPhuong", new Gson().toJson(mathchedRoute));
                    new DrawGeoJson(TimDuongDiFragmentBaoPhat.this, mathchedRoute).execute();
                }
            } else {
...
//        Toast.makeText(getContext(), mes, Toast.LENGTH_LONG).show();
    ...
    private static class MainActivityLocationCallback implements LocationEngineCallback<LocationEngineResult> {
        ...
        private final WeakReference<TimDuongDiFragmentBaoPhat> activityWeakReference;

        MainActivityLocationCallback(TimDuongDiFragmentBaoPhat activity) {
        ...
        public void onSuccess(LocationEngineResult result) {
            TimDuongDiFragmentBaoPhat activity = activityWeakReference.get();
            if (activity != null) {
    ...
    private static class DrawGeoJson extends AsyncTask<Void, Void, FeatureCollection> {
        ...
        private WeakReference<TimDuongDiFragmentBaoPhat> weakReference;


        DrawGeoJson(TimDuongDiFragmentBaoPhat activity, MathchedRoute mathchedRoute) {
        ...
        protected FeatureCollection doInBackground(Void... voids) {
            try {
                TimDuongDiFragmentBaoPhat activity = weakReference.get();
