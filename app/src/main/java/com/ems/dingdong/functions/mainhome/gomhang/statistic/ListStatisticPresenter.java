...
public class ListStatisticPresenter extends Presenter<ListStatisticContract.View, ListStatisticContract.Interactor>
    ...
    public void searchStatisticCollect(String postmanID, String fromDate, String toDate) {
        mView.showProgress();
        mInteractor.searchStatisticCollect(postmanID, fromDate, toDate, new CommonCallback<StatisticCollectResult>((Activity) mContainerView) {
            @Override
            protected void onSuccess(Call<StatisticCollectResult> call, Response<StatisticCollectResult> response) {
                super.onSuccess(call, response);
                mView.hideProgress();
                if (response.body().getErrorCode().equals("00")) {
                    mView.showResponseSuccess(response.body().getStatisticCollects());
                } else {
            ...
            @Override
            protected void onError(Call<StatisticCollectResult> call, String message) {
