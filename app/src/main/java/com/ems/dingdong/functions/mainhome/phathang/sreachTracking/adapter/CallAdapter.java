...
public class CallAdapter extends RecyclerView.Adapter<CallAdapter.HolderView> {


    List<StatusInfo> mList;
    Context mContext;

    public CallAdapter(Context context, List<StatusInfo> items) {
        mContext = context;
        mList = items;
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull CallAdapter.HolderView holder, int position) {
        holder.bindView(mList.get(position));

    }

    @Override
    public CallAdapter.HolderView onCreateViewHolder(ViewGroup parent, int viewType) {
        return new CallAdapter.HolderView(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_status_log, parent, false));
    }
