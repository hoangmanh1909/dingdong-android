...
public class DiaLogHoanTatTin4h extends Dialog {
    ...
    public DiaLogHoanTatTin4h(@NonNull Context context, String mess, int type, DialogHoanTTCallback idCallback) {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btn_khong:
                if (mType == Constants.GHI_NO)
                    idCallback.onResponse(Constants.GHI_NO);
                dismiss();
                break;
            case R.id.btn_co:
                idCallback.onResponse(Constants.TIEN_MAT);
                dismiss();
                break;
